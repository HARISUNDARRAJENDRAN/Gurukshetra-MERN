...
const Outcomes = () => {
  ...
  return (
    <section className="section bg-white relative overflow-hidden">
      <div className="absolute -bottom-32 left-1/2 h-72 w-72 -translate-x-1/2 rounded-full bg-slate-200/40 blur-3xl" />
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
          {stats.map((s) => (
            <div key={s.label} className="card p-6 text-center">
              ...
              <div className="mt-2 text-xs uppercase tracking-wide text-slate-500">{s.label}</div>
            </div>
          ))}
        </div>
      </div>
