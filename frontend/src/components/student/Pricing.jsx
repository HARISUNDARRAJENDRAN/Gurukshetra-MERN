          ...
          {plans.map((p) => (
            <div
              ...
              className={`p-6 transition-all duration-300 ${
                p.accent ? 'card-strong border-slate-900/10' : 'card hover:shadow-md'
              }`}
            >
              <div className="flex items-center justify-between">
                <h3 className="text-lg font-semibold text-slate-900">{p.name}</h3>
                {p.accent && (
                  <span className="text-xs font-semibold px-2 py-1 rounded-full bg-slate-900/10 text-slate-900">Popular</span>
                )}
              </div>
              <div className="mt-4">
                <span className="text-3xl font-bold text-slate-900">{p.price}</span>
                <span className="text-sm text-slate-500 ml-2">{p.period}</span>
              </div>
